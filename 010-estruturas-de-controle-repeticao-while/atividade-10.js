...
let verificacao = 0
while (verificacao == 0) {
    let numero1a5 = lerTeclado.questionInt(`Digite um número de 1 a 5: `)
    if (numero1a5 >= 1 & numero1a5 <= 5){
        console.log("Você escolheu: ", numero1a5);
        verificacao = 1
    } else {
...
// → Seu código aqui:
let verificacao2 = 0
let quaisNumerosDigitados = []
let numerosDigitados = 1
let somaNumerosDigitados = 0
while (verificacao2 == 0) {
    let numero = lerTeclado.questionInt(`Digite o número que eu escolhi: `)
    if (numero == 0){
        console.log("Acertou");
        verificacao2 = 1;
    } else {
        console.log("ERROUUUU KKKKKK.");
        numerosDigitados++;
        somaNumerosDigitados += numero;
        quaisNumerosDigitados.push(numero);
    }
}
console.log("Soma: ", somaNumerosDigitados, "Qtd numeros digitados: ", numerosDigitados, "Numeros digitos: ", quaisNumerosDigitados);
...
// → Seu código aqui:
console.log("Instrução: Digite notas de 0 a 10, e digite -1 para encerrar.");

let verificacao3 = 0
let notasDigitadas = []
let somaNotasDigitadas = 0
while (verificacao3 == 0) {
    ...
    if (nota == -1){
        ...
        numerosDigitados++;
        somaNotasDigitadas += nota;
        notasDigitadas.push(nota);
    }
}
console.log("Média: ", somaNotasDigitadas / notasDigitadas.length);
...
let horaData;
do {
    horaData = lerTeclado.questionInt(`
    1 - Exibir hora atual
    2 - Exibir data atual
    0 - Sair
...
while (palpite !== numeroSecreto) {
    palpite = lerTeclado.questionInt(`Digite o seu palpite: `)
    tentativas++
...
// → Seu código aqui:
let numeroInt = lerTeclado.questionInt(`Digite um número inteiro para exibir sua tabuda: `)
let i = 1
while (i <= 10) {
    let resultado = numeroInt * i
    console.log(`${numeroInt} x ${i} = ${resultado}`);
    i++
}
...
while (saldo > 50 & rodada < 10) {
    let saque = (Math.floor(Math.random() * 200) + 50)
    if(saque > saldo){
...
console.log(`Total de rodadas ${rodada} | Saldo restante: R$ ${saldo}`);
...
let verificacaao = 0
let totalcontatos = 0
while (verificacaao == 0) {
    totalcontatos++
    let nome = lerTeclado.question(`Digite o nome do contato: `)
    let telefone = lerTeclado.questionInt(`Digite o número do contato: `)
    contatos.push({nome: nome, telefone: telefone})
    let adicionarMais = lerTeclado.keyIn(`Adicionar outro contato?: `)
    if (!adicionarMais) {
        console.log("Encerrando...");
        verificacaao = 1
    }
